module ipx::dex {
  ...
  const ERROR_REMOVE_LIQUIDITY_ZERO_AMOUNT: u64 = 9;
    ...
    public fun create_pool<X, Y>(
      ...
      let (treasury, metadata) = coin::create_currency<LPCoin<X, Y>>(
            LPCoin {}, 
            9,
            b"IPX-LP",
            bcs::to_bytes(&lp_coin_name),
            b"Interest Protocol Liquidity Provider Token",
            option::some(url::new_unsafe(ascii::string(bcs::to_bytes(&b"https://sui.interestprotocol.com")))),
            ctx
        );
    ...
    fun get_amounts<X, Y>(pool: &Pool<X, Y>): (u64, u64, u64) {
        (
